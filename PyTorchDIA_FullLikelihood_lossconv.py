...
def infer_kernel(R, I, maxiter, FIM, convergence_plots, d, ks, speedy, tol, lr_kernel, lr_B, SD_steps, Newton_tol):
    ...
    * 'fit' (numpy.ndarray): the spatially varying background (inferred_bkg == fit if d=0)
    ...
    class log_likelihood(torch.nn.Module):
        ...
        def forward(model, targ):
            ...
            with torch.no_grad():
                ...
                var[var < 0.] = 1.
            NM = torch.sqrt(var)
            chi2 = torch.sum(torch.pow(((model-targ)/NM), 2))
    ...
    if speedy is True:
      ...
      #print('Will converge when relative change in loss <:', tol)
      ...
      #optimizer_Newton = torch.optim.LBFGS(model.parameters(), history_size=10, line_search_fn=None)     
      #def closure():
    ...
    print('Check dtype of weights and data')
    print(R.dtype, I.dtype, model[0].weight.dtype, model[0].bias.dtype)
    for t in range(maxiter):

        kernels.append(model[0].weight[0][0].cpu().detach().numpy())
        ...
        if use_Newton == False:
          if speedy is True:
            ...
            #print('%.23f' % loss.detach().item())
            #print(type(loss.detach().item()))
          else:
            y_pred = model(R, A)
            loss = -log_likelihood.forward(y_pred, I)
          ...
          def closure():
            ...
            #print(loss)
            loss.backward()
            #print('Epoch:%d, Loss:%.10f' % (t, loss.detach().item()))
            #print(type(loss.detach().item()))
            return loss

          #print('Newton step at iteration:', t)
          #print('Max number of possible steps:', SD_steps_taken + 249)
        ...
        if use_Newton == True:
            print('Newton step %d, Relative change in Loss=%.9f' % (t - SD_steps_taken, abs((losses[-1] - losses[-2])/losses[-2])))
        ...
        if speedy is True:
          ...
          if t>100 and abs((losses[-1] - losses[-2])/losses[-2]) < tol:
            print('Relative change in Loss: %.15f' % abs((losses[-1] - losses[-2])/losses[-2]))
            #print('Relative change in P: %.15f' % abs((psf[-1] - psf[-2])/psf[-2]))
            print('Last 3 losses: %.10f, %.10f, %.10f' % (losses[-1], losses[-2], losses[-3])) 
            print(psf[-1], B0[-1])
            print("'Speedy' convergence reached!")
            #print("SD steps taken:", SD_steps_taken)
            #print("Newton steps taken:", t - SD_steps_taken)
            #print("Converged in %d / %d possible iterations" % (t, SD_steps_taken + 50000-1))
            #plt.title('Relative Change in kernel between final two steps')
            #print((abs((kernels[-1] - kernels[-2])/kernels[-2]) < tol).all())
            #plt.imshow(abs((kernels[-1] - kernels[-2])/kernels[-2]))
            #plt.colorbar()
            #plt.show()
            break
            ...
            print('Relative change in Loss: %.9f' % abs((losses[-1] - losses[-2])/losses[-2]))
          elif t == maxiter - 1:
            print('Failed to converge!')
            failed = 1
            break
...
def DIA(R,
  ...
  * 'B' (numpy.ndarray): the differential background
  ...
  kernel, B, failed = infer_kernel(R, I, 
