    ...
    * 'fit' (numpy.ndarray): the spatially varying background (inferred_bkg == fit if d=0)
    ...
    for t in range(maxiter):
        ...
        if speedy is True:
          ...
          if t>200 and abs((losses[-1] - losses[-2])/losses[-2]) < tol:
            #print(psf[-1], B0[-1])
            print("'Speedy' convergence reached! (with guard against stopping too early)")
            ...
            #plt.show()
            break
          elif t>200 and abs((losses[-1] - losses[-2])/losses[-2]) < Newton_tol and use_Newton == False:
            use_Newton = True
            SD_steps_taken = t
